...
# =============================================================================
# Sprint 12: Enhanced Data Exploration Models
# =============================================================================
...
class DatasetInfo(BaseModel):
    ...
    Used in DataExplorationSummary to provide a quick overview of each
    dataset without the full profiling detail.
    """
    ...
    file_format: str = Field(default="unknown", description="File format (csv, parquet, etc.)")
    
    # Optional temporal info
    date_range_start: date | None = Field(
        default=None,
        description="Earliest date in time-based data"
    )
    date_range_end: date | None = Field(
        default=None,
        description="Latest date in time-based data"
    )
    
    # Structure detection
    structure_type: DataStructureType = Field(
        default=DataStructureType.UNKNOWN,
        description="Detected data structure type"
    )
    
    # Key columns identified
    key_columns: list[str] = Field(
        default_factory=list,
        description="Columns identified as key variables for research"
    )
    
    @property
    def date_range_str(self) -> str | None:
        """Human-readable date range string."""
        if self.date_range_start and self.date_range_end:
            return f"{self.date_range_start} to {self.date_range_end}"
        return None


class QualityFlagItem(BaseModel):
    """A single quality flag with context.
    
    Associates a QualityFlag enum value with specific details about
    where and why the flag was raised.
    """
    
    flag: QualityFlag = Field(..., description="The quality flag type")
    ...
    dataset_name: str | None = Field(
        default=None,
        description="Affected dataset (if specific to one)"
    )
    column_name: str | None = Field(
        default=None,
        description="Affected column (if specific to one)"
    )
    description: str = Field(..., description="Human-readable description")
    details: dict[str, Any] = Field(
        default_factory=dict,
        description="Additional details (counts, percentages, etc.)"
    )
...
class DataExplorationSummary(BaseModel):
    """High-level summary of data exploration with LLM-generated prose.
    
    This model provides a human-readable summary suitable for academic
    writing (Methods section), plus structured inventory and quality flags.
    Generated by the enhanced data_explorer node in Sprint 12.
    
    Example prose_description:
        "The uploaded datasets contain 46.2 million options contract records
        spanning 2004-2024 for GOOG, GOOGL, SPY, and VIX. Each dataset includes
        strike price, expiration date, bid/ask quotes, volume, and implied
        volatility. Data quality is high with <0.1% missing values."
    """
    ...
    # LLM-generated prose description (100-200 words)
    prose_description: str = Field(
        default="",
        description="Academic prose description suitable for Methods section"
    )
    
    # Structured dataset inventory
    dataset_inventory: list[DatasetInfo] = Field(
        default_factory=list,
        description="Inventory of all datasets with key statistics"
    )
    
    # Quality assessment
    quality_flags: list[QualityFlagItem] = Field(
        default_factory=list,
        description="Quality issues detected during exploration"
    )
    overall_quality: DataQualityLevel = Field(
        default=DataQualityLevel.NOT_ASSESSED,
        description="Overall data quality assessment"
    )
    
    # Relevance to research
    recommended_variables: list[str] = Field(
        default_factory=list,
        description="Variables recommended for the research question"
    )
    data_gaps: list[str] = Field(
        default_factory=list,
        description="Data gaps identified (what's missing for research)"
    )
    
    # Aggregate statistics
    total_rows: int = Field(default=0, ge=0, description="Total rows across all datasets")
    total_columns: int = Field(default=0, ge=0, description="Total unique columns")
    total_memory_mb: float = Field(default=0.0, ge=0.0, description="Total memory usage")
    
    # File read errors (files that couldn't be processed)
    file_read_errors: list[str] = Field(
        default_factory=list,
        description="Error messages for files that failed to load"
    )
    
    @property
    def dataset_count(self) -> int:
        """Number of datasets in inventory."""
        return len(self.dataset_inventory)
    
    @property
    def has_quality_issues(self) -> bool:
        """Check if any quality flags were raised."""
        return len(self.quality_flags) > 0
    
    @property
    def critical_flags(self) -> list[QualityFlagItem]:
        """Get only critical severity flags."""
        return [f for f in self.quality_flags if f.severity == CritiqueSeverity.CRITICAL]
    ...
    def get_flags_by_severity(self, severity: CritiqueSeverity) -> list[QualityFlagItem]:
        """Get flags filtered by severity."""
        return [f for f in self.quality_flags if f.severity == severity]
...
class SectionWritingContext(BaseModel):
    ...
    # Sprint 16: Tables and figures from data analyst
    tables: list["TableArtifact"] = Field(
        default_factory=list,
        description="Table artifacts generated by data analyst"
    )
    figures: list["FigureArtifact"] = Field(
        default_factory=list,
        description="Figure artifacts generated by data analyst"
    )
    
    # Sprint 16: Data exploration prose summary for methods section
    data_exploration_prose: str = Field(
        default="",
...
# =============================================================================
# Sprint 14: Data Acquisition Models
# =============================================================================


class TimeRange(BaseModel):
    """Time range specification for data requirements."""
...
class DataRequirement(BaseModel):
    """A single data requirement for the research analysis.
    
    Specifies what data is needed, including the type, time range,
    entities, and priority level.
    """
    ...
    variable_name: str = Field(
        ...,
        description="Name of the variable needed (e.g., 'daily_returns')"
    )
    data_type: str = Field(
        ...,
        description="Type of data (e.g., 'stock_prices', 'economic_indicator')"
    )
    description: str = Field(
        default="",
        description="Human-readable description of the data needed"
    )
    required_fields: list[str] = Field(
        default_factory=list,
        description="Specific fields needed (e.g., ['close', 'volume'])"
    )
    ...
    entities: list[str] | None = Field(
        default=None,
        description="Specific entities needed (e.g., ['AAPL', 'GOOG'])"
    )
    priority: DataRequirementPriority = Field(
        default=DataRequirementPriority.REQUIRED,
        description="Priority level of this requirement"
    )
    
    # Matching with available data
    matched_dataset: str | None = Field(
        default=None,
        description="Name of dataset that satisfies this requirement"
    )


class DataAcquisitionTask(BaseModel):
    """A task to acquire specific data from an external source."""
    ...
    requirement: DataRequirement = Field(
        ...,
        description="The data requirement this task fulfills"
    )
    source: str = Field(
        ...,
        description="Data source name (e.g., 'yfinance', 'fred')"
    )
    tool_to_use: str = Field(
        ...,
        description="Tool function to call (e.g., 'acquire_stock_data')"
    )
    ...
    status: AcquisitionStatus = Field(
        ...
        description="Current status of the acquisition"
    )
    result_dataset: str | None = Field(
        default=None,
        description="Name of resulting dataset if successful"
    )
...
class DataAcquisitionPlan(BaseModel):
    """Plan for acquiring all needed data for the research.
    
    Generated by the data acquisition node after analyzing
    the research plan's data requirements.
    """
    ...
    requirements: list[DataRequirement] = Field(
        default_factory=list,
        description="All data requirements from research plan"
    )
    available_in_upload: list[str] = Field(
        default_factory=list,
        description="Requirements satisfied by uploaded data"
    )
    to_acquire: list[DataAcquisitionTask] = Field(
        default_factory=list,
        description="Tasks for data that needs to be fetched"
    )
    
    # Timing
    estimated_time: str = Field(
        default="unknown",
        description="Estimated time to complete acquisition"
    )
    ...
    def requirements_met(self) -> int:
        """Number of requirements already met."""
        return len(self.available_in_upload)
    
    @property
    def requirements_to_fetch(self) -> int:
        """Number of requirements that need fetching."""
        return len(self.to_acquire)


class AcquisitionFailure(BaseModel):
    """Record of a failed data acquisition attempt.
    
    Tracks which sources were tried and what went wrong,
    plus suggested actions for the user.
    """
    ...
    requirement: DataRequirement = Field(
        ...,
        description="The data requirement that could not be fulfilled"
    )
    attempted_sources: list[str] = Field(
        default_factory=list,
        description="Sources that were tried"
    )
    error_messages: list[str] = Field(
        default_factory=list,
        description="Error messages from each attempt"
    )
    user_action_needed: str = Field(
        default="",
...
class CodeSnippet(BaseModel):
    """A code snippet generated for data acquisition.
    
    Used when built-in tools cannot satisfy a requirement
    and custom code needs to be generated.
    """
    ...
    requirement_id: str | None = Field(
        default=None,
        description="ID of requirement this code fulfills"
    )
    
    # Execution results
    status: CodeExecutionStatus = Field(
        default=CodeExecutionStatus.NOT_EXECUTED,
...
class AcquiredDataset(BaseModel):
    """Record of a successfully acquired dataset.
    
    Tracks where the data came from and links to the
    original requirement.
    """
    
    dataset_name: str = Field(
        ...,
        description="Name registered in DataRegistry"
    )
    source: str = Field(
        ...,
        description="Source name (e.g., 'yfinance', 'fred')"
    )
    requirement_id: str = Field(
        ...,
        description="ID of the requirement this satisfies"
    )
    acquired_at: datetime = Field(
        default_factory=_utc_now,
        description="When the data was acquired"
    )
    row_count: int = Field(
        default=0,
